...
import SieclePage from './SieclePage'
...
function SortiesPage() {
    ...
    const getData = async (int) => {
        const r = await fetch('http://127.0.0.1:8000/guest/getRescueReport/20', {
            method: 'GET'
        })
        const rj = await r.json()
        setResponse(rj.rescueReport)
        setSelected(int)
    }
    ...
    return (
        <div className="description flex flex-wrap flex-col content-center mt-8 mb-8 shadow-md pb-16">
            ...
            {selected && <SieclePage siecle={selected} rescues={response                                                                         }/>}
